						...
						<table id="mytable" class="table table-bordred table-striped">
							...
							<thead>
								<tr>
									...
									<th class="col-sm-1" id="name">이름</th>
									<th class="col-sm-3" id="contents" align="center">내용</th>
									<th class="col-sm-1" id="regDate">등록일</th>
								</tr>
							</thead>
							...
							<tbody id="table">
								...
								<c:forEach var="message" items="${list}">
									...
									<tr>
										...
										<td>${message.temp}</td>
										<td>
										<c:if test="${message.tranCode eq '1'}">
										<b><a href="getMessage.do?messageNo=${message.messageNo}&userName=${message.temp}&userNo=${message.sendUser}">${message.contents}</a></b>
										</c:if>
										<c:if test="${message.tranCode eq '2'}">
										<a href="getMessage.do?messageNo=${message.messageNo}&userName=${message.temp}&userNo=${message.sendUser}">${message.contents}</a>                
										</c:if>
										</td>
