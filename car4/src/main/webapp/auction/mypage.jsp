...
<td colspan="11" >전체  <%= resultPage.getTotalCount() %> 건수,현재 <%= resultPage.getCurrentPage() %> 페이지</td>--%>
							...
							<c:forEach var="auction" items="${auction}">
								...
								<tr align="left">
									<td>${ i }</td>
									<td>${auction.model }</td>
									<td><fmt:formatDate value="${auction.regDate}"
											pattern="MM / dd"></fmt:formatDate></td>
									<td>${auction.title }</td>
									<td><c:if test="${auction.successCar == '0' }">
											<a type="button" class="btn btn-primary"
												href="getAuctionView.do?auctionNo=${auction.auctionNo }">경매중</a>
										</c:if> <c:if test="${auction.successCar != '0' }">
